...
} = require("../utils/localDateTimeUtils");
...
tripItemRouter.post(
    ...
    async (request, response) => {
        try {
            ...
            if (
                status !== undefined &&
                !isValidEnumValue(status, tripItemStatusValues)
            ) {
                return response.status(400).json({
            ...
            if (!isValidLocalDateTime(startDateTime)) {
                return response.status(400).json({
                    message:
            ...
            if (endDateTime && !isValidLocalDateTime(endDateTime)) {
                return response.status(400).json({
                    message:
                        "endDateTime must be a valid local datetime in YYYY-MM-DDTHH:mm format."
                });
            }

            if (
                endDateTime &&
                !isStartBeforeOrEqualEnd(startDateTime, endDateTime)
            ) {
                return response.status(400).json({
                    message:
...
tripItemRouter.get(
    ...
    async (request, response) => {
        try {
            ...
            const tripItems = await TripItem.find(tripItemQuery).sort({
                startDateTime: 1
            });
        ...
        try {
            ...
            const nextStartDateTime =
                startDateTime !== undefined
                    ? startDateTime
                    : foundTripItem.startDateTime;
            const nextEndDateTime =
                endDateTime !== undefined
                    ? endDateTime
                    : foundTripItem.endDateTime;

            if (
                startDateTime !== undefined &&
                !isValidLocalDateTime(startDateTime)
            ) {
                return response.status(400).json({
                    message:
                        "startDateTime must be a valid local datetime in YYYY-MM-DDTHH:mm format."
            ...
            if (
                endDateTime !== undefined &&
                endDateTime &&
                !isValidLocalDateTime(endDateTime)
            ) {
                return response.status(400).json({
                    message:
                        "endDateTime must be a valid local datetime in YYYY-MM-DDTHH:mm format."
            ...
            if (
                nextEndDateTime &&
                !isStartBeforeOrEqualEnd(nextStartDateTime, nextEndDateTime)
            ) {
                return response.status(400).json({
                    message:
                        "startDateTime must be before or equal to endDateTime."
